...
const SearchBar = () => {
  ...
  const fetchPartNumberData = async (
    ...
    const endpoint = `http://127.0.0.1:8000/part-number?query=${encodeURIComponent(
      trimmedQuery
  ...
  const fetchModelData = async (
    ...
    setSelectedModelItems([]); // Clear selected items for model search
    const endpoint = `http://127.0.0.1:8000/model?query=${encodeURIComponent(
      trimmedQuery
    )}&country=${encodeURIComponent(
      currentCountry || ""
    )}&region=${encodeURIComponent(currentRegion || "")}`;
  ...
  const openSelectedPartsPage = async () => {
    ...
    try {
      const endpoint = "http://127.0.0.1:8000/part-number/multiple";
      const postData = {
